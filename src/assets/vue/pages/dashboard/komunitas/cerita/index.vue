<template>
  <f7-page>
    ...
    <page-list
      ...
      :api="apiJson.API + 'api/v1/user/cerita'"
    ...
    >
      ...
      <template v-slot:additional>
        <f7-card
          style="background-color: white;position: fixed; bottom: 0px;left: 0; right: 0; z-index: 99999"
        >
          <f7-row>
            <f7-col
              :width="50"
              @click.native="$f7router.navigate('/cerita/my')"
            >
            ...
            <f7-col
              :width="50"
              @click="$f7router.navigate('/cerita/create')"
            >
              <p class="text-center text-primary">
                Buat Baru
              </p>
        ...
        >
          ...
          <div class="item-inner">
            ...
            <div class="item-subtitle">
              <small v-html="data.reviewed_content" />
            </div>
          </div>
        </div>
      </template>
...
export default {
  ...
  methods: {
    onCeritaSaya() {
      console.log('CLICKED CERITA SAYA')
    },
    init(q) {
      this.q_status = q
    ...
    onSelected(selected) {
      this.$f7router.navigate('/cerita/' + selected.id)
    }
  }
}
</script>